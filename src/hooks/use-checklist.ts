...
export const useChecklist = () => {
  ...
  // 기본 체크리스트 템플릿 생성
  const createDefaultTemplate = useCallback(async (): Promise<string> => {
    try {
  ...
  // 템플릿 가져오기
  const getTemplate = useCallback(async (): Promise<ChecklistTemplate | null> => {
    try {
      const q = query(
      ...
      if (snapshot.empty) {
        // 템플릿이 없으면 기본 템플릿 생성
        const templateId = await createDefaultTemplate();
      ...
      const updatedItems = checklist.items.map(item => {
        if (item.itemId === itemId) {
          return {
            ...item,
            checked,
            checkedAt: checked ? serverTimestamp() as Timestamp : undefined,
